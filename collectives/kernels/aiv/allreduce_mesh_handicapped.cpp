 ...
 */
/**
 * Mesh AllReduce kernel — HANDICAPPED variant (performance idea #2 A/B test).
 *
 * Identical to allreduce_mesh.cpp EXCEPT it reproduces the two codegen defects the
 * pypto *generated* composite kernel emits (see build_output/.../reduce_step.cpp):
 *
 *   1. `pipe_barrier(PIPE_ALL)` before EVERY peer TNOTIFY in the Phase-2 notify loop.
 *   2. `dcci((void*)0, ENTIRE_DATA_CACHE)` after EVERY peer TWAIT in the Phase-2 wait loop.
 *
 * Purpose: attribute the measured 6.7x on-device gap between the hand-written
 * simpler-own kernel (0.132 ms @64K P=2) and the pypto composite (0.888 ms @64K P=2)
 * to these per-peer full barriers + whole-cache invalidates. If this kernel slows to
 * ~composite levels, the fix for idea #2 is "remove them" (target: back to 0.13 ms).
 */
