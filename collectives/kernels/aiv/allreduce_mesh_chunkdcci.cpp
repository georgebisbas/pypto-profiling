 ...
 */
/**
 * Mesh AllReduce kernel — CHUNK-DCCI ABLATION (plan 107).
 *
 ...
 *
 *   dcci(0, ENTIRE_DATA_CACHE) per peer inside the per-chunk loop.
 *   Generated: reduce_step.before.cpp:530. Baseline: zero dcci.
 *
...
extern "C" __aicore__ __attribute__((always_inline)) void kernel_entry(__gm__ int64_t *args) {
    ...
    for (int offset = 0; offset < count; offset += kChunkCols) {
        ...
        for (int peer = 0; peer < nranks; ++peer) {
            ...
            // ABLATION: generated composite invalidates the ENTIRE data cache once
            // per peer inside the per-chunk wait loop
            // (reduce_step.before.cpp:530). Baseline has zero dcci.
