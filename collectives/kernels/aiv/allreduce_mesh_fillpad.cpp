 ...
 */
/**
 * Mesh AllReduce kernel — FILLPAD ABLATION (plan 107).
 *
 * Identical to allreduce_mesh.cpp EXCEPT:
 *
 *   Vec tiles carry PadValue::Zero and each peer chunk is TFILLPAD'd before
 *   the TADD, exactly as the generated composite does
 *   (reduce_step.before.cpp:364 dst v113 PadValue::Zero, :456 the call,
 *   :465 the TADD over padded tiles). The baseline uses PadValue::Null
 *   tiles and zero TFILLPAD. Tile count and UB budget are unchanged.
 *
 ...
 * (chip run lane poisoned, code -100). Slot order is cost-neutral, so the
 * one-construct delta to mesh is unchanged.
 *
 * Purpose: attribute a share of the measured 6.7x on-device gap between the
 * hand-written kernel (0.132 ms @64K P=2) and the pypto composite (0.888 ms
 * @64K P=2) to that construct.
 */
...
extern "C" __aicore__ __attribute__((always_inline)) void kernel_entry(__gm__ int64_t *args) {
    ...
    // UB layout (safety constraint, see header): recvTile is the ONLY tile that
    // receives TFILLPAD<InPlace>, and a FULL-width (kChunkCols == 16384) fillpad's
    ...
    // fillpad'd) takes the top slot. Same as the composite: its fillpad'd tile
    // regions end at <= 0xC000, well inside UB.
