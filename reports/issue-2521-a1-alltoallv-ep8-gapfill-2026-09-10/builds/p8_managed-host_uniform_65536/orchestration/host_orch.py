...
def host_orch(orch, _args, config, *, tensors, callables, sub_ids, _keep, world_size, _domain_provider=None):
    with (_domain_provider or orch.allocate_domain)(
        ...
        window_size=((((524288 + 31) // 32) * 32)) + ((((524288 + 31) // 32) * 32)) + ((((32 + 31) // 32) * 32)) + ((((32 + 31) // 32) * 32)) + ((((32 + 31) // 32) * 32)),
        buffers=[
        ...
        for r__idx_v0 in range(0, t__tmp_v0, 1):
            tensors["t__tmp_v1"] = tensors["inputs__ssa_v0"][r__idx_v0, 0:8, 0:65536]
            _ta_0 = TaskArgs()
            _ta_0.add_tensor(make_tensor_arg(orch._worker, tensors["t__tmp_v1"]), TensorArgType.INPUT)
            _ta_0.add_tensor(__comm_d0[r__idx_v0].buffers["input_buf__ssa_v0"].tensor(shapes=(8, 65536), dtype=DataType.INT8), TensorArgType.OUTPUT_EXISTING)
        ...
        for r in range(0, world_size, 1):
            _ta_2 = TaskArgs()
            _ta_2.add_tensor(__comm_d0[r].buffers["input_buf__ssa_v0"].tensor(shapes=(8, 65536), dtype=DataType.INT8), TensorArgType.INPUT)
            _ta_2.add_tensor(__comm_d0[r].buffers["data_buf__ssa_v0"].tensor(shapes=(8, 65536), dtype=DataType.INT8), TensorArgType.INOUT)
        ...
        for r__idx_v0 in range(0, t__tmp_v4, 1):
            tensors["t__tmp_v5"] = tensors["outputs__ssa_v0"][r__idx_v0, 0:8, 0:65536]
            tensors["t__tmp_v6"] = tensors["recv_outputs__ssa_v0"][r__idx_v0, 0:8, 0:1]
            _ta_3 = TaskArgs()
            _ta_3.add_tensor(__comm_d0[r__idx_v0].buffers["data_buf__ssa_v0"].tensor(shapes=(8, 65536), dtype=DataType.INT8), TensorArgType.INPUT)
